...
export const NavigationWaypoint = ({
  ...
  const getStepIcon = (icon: string) => {
    switch (icon) {
      case 'left':
        return <svg width="32" height="32" viewBox="0 0 24 24" fill="none" className="text-white">
      ...
      case 'arrive':
        return <Flag className="text-white" size={32} strokeWidth={2.5} />;
      default:
        return <Navigation className="text-white" size={32} />;
    }
  };
  return <div className="fixed inset-0 z-[100] bg-zinc-50 flex flex-col font-sans overflow-hidden">
      {/* Header */}
      <div className="relative bg-white shadow-sm px-6 py-4 z-20">
        <div className="flex items-center justify-between">
          <button onClick={onClose} className="p-2 hover:bg-zinc-100 rounded-xl transition-colors">
            <ArrowLeft className="text-zinc-800" size={24} />
          </button>
          
          <div className="flex-1 text-center">
            <h2 className="text-lg font-semibold text-zinc-800">Navigation</h2>
          </div>

          <button onClick={() => setVoiceEnabled(!voiceEnabled)} className="p-2 hover:bg-zinc-100 rounded-xl transition-colors">
            {voiceEnabled ? <Volume2 className="text-[#005C4B]" size={24} /> : <VolumeX className="text-zinc-400" size={24} />}
          </button>
        </div>

        {/* Progress Bar */}
        <div className="mt-4 relative h-2 bg-zinc-100 rounded-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto pb-32">
        {/* Minimap Preview */}
        <div className="relative w-full h-64 bg-gradient-to-br from-zinc-100 to-zinc-200 overflow-hidden">
          {/* Map Grid Pattern */}
          ...
          <svg className="absolute inset-0 w-full h-full">
            ...
            {/* Destination Marker */}
            <circle cx="230" cy="40" r="8" fill="#8b5cf6" />
          </svg>
          ...
          <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl px-4 py-2 shadow-lg">
              <div className="flex items-center gap-2">
                <Clock className="text-[#005C4B]" size={16} />
                <span className="text-sm font-semibold text-zinc-800">{timeRemaining} min</span>
              </div>
            </div>
            
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl px-4 py-2 shadow-lg">
              <div className="flex items-center gap-2">
                <Navigation className="text-[#005C4B]" size={16} />
                <span className="text-sm font-semibold text-zinc-800">{distanceRemaining}m</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          <AnimatePresence mode="wait">
            ...
            duration: 0.3
          }} className="bg-[#2A1805] rounded-[2.5rem] p-8 mb-6 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-16 h-16 bg-[#005C4B] rounded-2xl flex items-center justify-center shrink-0">
                    {getStepIcon(directionSteps[currentStep].icon)}
                  </div>
                  
                  <div className="flex-1">
                    <div className="text-[#DDF247] text-sm font-semibold uppercase tracking-wide mb-1">
                ...
                <div className="flex items-center gap-2">
                  <div className="h-1 w-16 bg-[#DDF247] rounded-full" />
          ...
          <div className="bg-white rounded-[2.5rem] p-6 shadow-lg mb-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-2 flex-wrap mt-3">
                  {destination.type.map(type => <span key={type} className="px-3 py-1 bg-[#DDF247]/30 text-[#005C4B] text-xs font-semibold rounded-full">
                      {type}
                    </span>)}
                </div>
              </div>
          ...
          <div className="space-y-3">
            ...
            delay: index * 0.1
                ...
                <div className="w-10 h-10 bg-zinc-100 rounded-xl flex items-center justify-center shrink-0">
                  {step.icon === 'left' && <ArrowLeft className="text-zinc-600" size={20} />}
                  {step.icon === 'right' && <ArrowRight className="text-zinc-600" size={20} />}
                  {step.icon === 'straight' && <ArrowUpRight className="text-zinc-600" size={20} />}
                  {step.icon === 'arrive' && <Flag className="text-zinc-600" size={20} />}
                </div>
      ...
      <AnimatePresence>
        ...
        opacity: 0
            ...
            <div className="bg-[#DDF247] rounded-[3rem] px-12 py-8 shadow-2xl text-center">
              <Flag className="text-[#005C4B] mx-auto mb-3" size={48} />
              <h2 className="text-3xl font-bold text-[#2A1805] mb-2">
                Almost There!
              </h2>
              <p className="text-[#2A1805]/80 font-medium">
